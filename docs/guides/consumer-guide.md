...
| `rag-retrieval` | `RuleRagRetrievalWorker` -- `SocRagRetrieveService` queries neocortex `CaseContextRetriever` across ATT&CK reference corpus + per-tenant internal knowledge corpus (incident post-mortems ingested via `SocKnowledgeIngestor` at case resolution) | — | retrievedChunks[], summary |
